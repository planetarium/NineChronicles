...
namespace Nekoyume.UI.Module
{
    ...
    public class ShopItemView : MonoBehaviour
    {
        ...
        public void Set(ShopItem model, Action<ShopItem> onClick)
        {
            ...
            if (model.OrderDigest.Quantity > 1)
            {
                var priceText = decimal.Round(model.OrderDigest.Price / model.OrderDigest.Quantity, 3);
                baseItemView.PriceText.text = $"{model.OrderDigest.Price}({priceText})";
            }
            else
            {
                baseItemView.PriceText.text = model.OrderDigest.Price.ToString();
            }
