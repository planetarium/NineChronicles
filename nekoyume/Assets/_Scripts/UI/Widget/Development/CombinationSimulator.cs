...
namespace Nekoyume.UI
{
    public class CombinationSimulator : Widget
    {
        ...
        private async void AsyncCombinationSimulate()
        {
            ...
            NcDebugger.Log($"-------------S T A R T (Combination) [COUNT] : {count}----------");
            var task = Task.Run(() =>
            {
            ...
            if (finish)
            {
                NcDebugger.Log("-------------F I N I S H (Combination)----------");
            }
        }
        ...
        private static void DrawResult(int itemId, int count, Result result)
        {
            ...
            NcDebugger.Log($"[CS] [{L10nManager.Localize($"ITEM_NAME_{itemId}")}] {itemId} / " +
        ...
        {
            var count = int.Parse(inputField.text);
            NcDebugger.Log($"-------------S T A R T (Enhancement) [COUNT] : {count}----------");
            var task = Task.Run(() =>
            {
...
*/
            ...
            if (finish)
            {
                NcDebugger.Log("-------------F I N I S H (Enhancement)----------");
            }
        }
    }
}
