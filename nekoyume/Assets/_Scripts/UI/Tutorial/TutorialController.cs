...
namespace Nekoyume.UI
{
    public class TutorialController
    {
        ...
        public TutorialController(IEnumerable<Widget> widgets)
        {
            foreach (var widget in widgets)
            {
                ...
                foreach (var target in widget.tutorialTargets.Where(target => target != null))
                {
                    ...
                    {
                        NcDebugger.LogError($"Duplication Tutorial Targets AlreadyRegisterd : {_targets[target.type].gameObject.name}  TryRegisterd : {target.rectTransform.gameObject.name}");
                        continue;
                    }
                ...
                foreach (var action in widget.tutorialActions)
                {
                    ...
                    if (methodInfo != null)
                    {
                        ...
                        {
                            NcDebugger.LogError($"Duplication Tutorial {action} Action AlreadyRegisterd : {_actions[action].ActionWidget.name}  TryRegisterd : {widget.name}");
                            continue;
                        }
        ...
        {
            NcDebugger.Log($"[TutorialController] Play({id})");
