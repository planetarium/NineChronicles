...
$@"query {{
    ...
    policy(level: {level}, free: true) {{
        ...
        rewards {{
            ...
            if (response is null)
            {
                NcDebugger.LogError($"Failed getting response : {nameof(InitializeResponse)}");
                return;
            }
            ...
            {
                NcDebugger.LogError($"Failed getting response : {nameof(PolicyResponse.Policy)}");
            }
            else
            {
        ...
        public async Task LoadAvatarInfo(string avatarAddress, string agentAddress)
        {
            ...
            if (response is null)
            {
                NcDebugger.LogError($"Failed getting response : {nameof(AvatarResponse)}");
                return;
            }
...
$@"query {{
    policy(level: {level}, free: true) {{
        ...
        rewards {{
            ...
            if (response is null)
            {
                NcDebugger.LogError($"Failed getting response : {nameof(PolicyResponse)}");
                return;
            }
...
$@"mutation {{
    claim(avatarAddress: ""{avatarAddress}"", agentAddress: ""{agentAddress}"")
            ...
            if (response is null)
            {
                NcDebugger.LogError($"Failed getting response : {nameof(ClaimResponse)}");
                return null;
            }
...
$@"mutation {{
    putAvatar(avatarAddress: ""{avatarAddress}"", agentAddress: ""{agentAddress}"") {{
        ...
        level
            ...
            if (response is null)
            {
                NcDebugger.LogError($"Failed getting response : {nameof(PutAvatarResponse)}");
                return;
            }

            if (response.PutAvatar is null)
            {
                NcDebugger.LogError($"Failed getting response : {nameof(PutAvatarResponse.PutAvatar)}");
                return;
            }
