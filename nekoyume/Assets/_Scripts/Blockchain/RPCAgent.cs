...
namespace Nekoyume.Blockchain
{
    ...
    public class RPCAgent : MonoBehaviour, IAgent, IActionEvaluationHubReceiver
    {
        ...
        {
            NcDebugger.Log($"[RPCAgent] Start initialization: {options.RpcServerHost}:{options.RpcServerPort}");
            PrivateKey = privateKey;
            ...
            if (_genesis == null)
            {
                ...
                sw.Stop();
                NcDebugger.Log($"[RPCAgent] genesis block imported in {sw.ElapsedMilliseconds}ms.(elapsed)");
            }
            ...
            NcDebugger.Log($"[RPCAgent] Finish initialization");
        }
        ...
        public async Task<IValue> GetStateAsync(Address accountAddress, Address address)
        {
            ...
            {
                NcDebugger.LogError("Failed to get tip block hash.");
                return null;
            }
        ...
        public async Task<IValue> GetStateAsync(long blockIndex, Address accountAddress, Address address)
        {
            ...
            if (!blockHash.HasValue)
            {
                NcDebugger.LogError($"Failed to get block hash from block index: {blockIndex}");
                return null;
            }
            ...
            Currency currency)
        {
            ...
            {
                NcDebugger.LogError($"Failed to get tup block hash.");
                return 0 * currency;
            }
        ...
        private AgentState ResolveAgentState(byte[] raw)
        {
            ...
            {
                NcDebugger.LogError("Given raw is not a format of the AgentState.");
                return null;
            }
        }
        ...
        private AvatarState ResolveAvatarState(byte[] raw)
        {
            ...
            {
                NcDebugger.LogError("Given raw is not a format of the AvatarState.");
                return null;
            }
            ...
            else if (full.Count == 4)
            {
                ...
                {
                    NcDebugger.LogError("Given raw is not a format of the inventory.");
                    return null;
                }

                if (full[2] is not Dictionary questListDict)
                {
                    NcDebugger.LogError("Given raw is not a format of the questList.");
                    return null;
                }

                if (full[3] is not Dictionary worldInformationDict)
                {
                    NcDebugger.LogError("Given raw is not a format of the worldInformation.");
                    return null;
                }
        ...
        public async Task<Dictionary<Address, IValue>> GetSheetsAsync(IEnumerable<Address> addressList)
        {
            ...
            await chunks
                ...
                .ParallelForEachAsync(async list =>
                {
                    ...
                    await raw.ParallelForEachAsync(async pair =>
                    {
                        ...
                        NcDebugger.Log($"[GetSheets/{new Address(pair.Key)}] buffer size: {size}");
                        await Task.CompletedTask;
                    });
                });
            sw.Stop();
            NcDebugger.Log($"[SyncTableSheets/GetSheets] Get sheets. {sw.Elapsed}");
            sw.Restart();
            var result = new Dictionary<Address, IValue>();
            ...
            sw.Stop();
            NcDebugger.Log($"[SyncTableSheets/GetSheets] decode values. {sw.Elapsed}");
            return result;
        }
        ...
        private void Awake()
        {
            ...
            OnRetryAttempt
                ...
                .Subscribe(tuple =>
                {
                    NcDebugger.Log($"Retry rpc connection. (remain count: {tuple.retryCount})");
        ...
        private IEnumerator CoTxProcessor()
        {
            ...
            while (true)
            {
                ...
                NcDebugger.Log($"[RPCAgent] CoTxProcessor()... before MakeTransaction.({++i})");
                Task task = Task.Run(async () =>
                {
                    await MakeTransaction(new List<ActionBase> { action });
                });
                yield return new WaitUntil(() => task.IsCompleted);
                NcDebugger.Log("[RPCAgent] CoTxProcessor()... after MakeTransaction." +
        ...
        private async Task MakeTransaction(List<ActionBase> actions)
        {
            ...
            NcDebugger.Log("[RPCAgent] MakeTransaction()... w/" +
                      $" nonce={nonce}" +
        ...
        private async void RetryRpc()
        {
            ...
            while (retryCount > 0)
            {
                ...
                try
                {
                    NcDebugger.Log($"Trying to join hub...");
                    await Join(true);
                    NcDebugger.Log($"Join complete! Registering disconnect event...");
                    RegisterDisconnectEvent(_hub);
                    UpdateSubscribeAddresses();
                ...
                {
                    NcDebugger.LogWarning($"TimeoutException occurred. Retrying... {retryCount}\n{toe}");
                    retryCount--;
                }
                catch (AggregateException ae)
                {
                    ...
                    {
                        NcDebugger.LogWarning($"RpcException occurred. Retrying... {retryCount}\n{re}");
                        retryCount--;
                    }
                    else
                    {
                        NcDebugger.LogWarning($"Unexpected error occurred during rpc connection. {ae}");
                        break;
                    }
                }
                catch (ObjectDisposedException ode)
                {
                    NcDebugger.LogWarning($"ObjectDisposedException occurred. Retrying... {retryCount}\n{ode}");
                    retryCount--;
                }
                catch (Exception e)
                {
                    NcDebugger.LogWarning($"Unexpected error occurred during rpc connection. {e}");
                    break;
                }
            }
        ...
        public void OnReorged(byte[] oldTip, byte[] newTip, byte[] branchpoint)
        {
            ...
            NcDebugger.Log($"[{nameof(RPCAgent)}] Render reorg: {BlockIndex}, {BlockTipHash.ToString()}");
        }
        ...
        public void OnException(int code, string message)
        {
            ...
            NcDebugger.Log($"{message} (code: {code})");
        ...
        public void OnPreloadStart()
        {
            ...
            NcDebugger.Log($"On Preload Start");
        }

        public void OnPreloadEnd()
        {
            OnPreloadEnded.OnNext(this);
            NcDebugger.Log($"On Preload End");
        }
        ...
        public void UpdateSubscribeAddresses()
        {
            ...
            NcDebugger.Log($"Subscribing addresses: {string.Join(", ", addresses)}");
