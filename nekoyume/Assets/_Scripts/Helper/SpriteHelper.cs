...
namespace Nekoyume.Helper
{
    public static class SpriteHelper
    {
        ...
        {
            return AddressablesLoader.Load<Sprite>(string.Format(CharacterIconPathFormat, characterId)) ??
        ...
        {
            return AddressablesLoader.Load<Sprite>(string.Format(ItemBackgroundPathFormat, grade)) ??
        ...
        public static Sprite GetBuffIcon(string iconResource)
        {
            ...
            return AddressablesLoader.Load<Sprite>(string.Format(BuffIconPathFormat, iconResource)) ??
                   AddressablesLoader.Load<Sprite>(BuffIconDefaultPath);
        }

        public static Sprite GetPlayerSpineTextureWeapon(int equipmentId)
        {
            return AddressablesLoader.Load<Sprite>(string.Format(PlayerSpineTextureWeaponPathFormat, equipmentId)) ??
                   AddressablesLoader.Load<Sprite>(string.Format(PlayerSpineTextureWeaponPathFormat, GameConfig.DefaultAvatarWeaponId));
        }

        public static Sprite GetAreaAttackCutsceneSprite(int id)
        {
            return AddressablesLoader.Load<Sprite>($"{AreaAttackCutscenePath}{id}") ??
                   AddressablesLoader.Load<Sprite>($"{AreaAttackCutscenePath}{GameConfig.DefaultAvatarArmorId}");
        }

        public static Sprite GetRankIcon(int rank)
        {
            return AddressablesLoader.Load<Sprite>(string.Format(RankIconPath, rank.ToString("D2")));
        }
        ...
        public static Sprite GetMailIcon(MailType mailType)
        {
            ...
            var result = AddressablesLoader.Load<Sprite>(string.Format(MailIconPathFormat, fileName));
