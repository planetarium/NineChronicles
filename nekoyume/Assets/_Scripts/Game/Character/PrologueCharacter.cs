...
namespace Nekoyume.Game.Character
{
    ...
    public class PrologueCharacter : MonoBehaviour
    {
        ...
        public IEnumerator CoBlowAttack(ElementalType elementalType)
        {
            ...
            var effect = Stage.instance.SkillController.Get<SkillBlowVFX>(_target.gameObject, elementalType, SkillCategory.BlowAttack, SkillTargetType.Enemies);
            effect.Play();
            for (var i = 0; i < 5; i++)
            {
                var sec = i == 0 ? 0 : i / 10f;
        ...
        protected virtual IEnumerator CoAnimationCast(ElementalType elementalType)
        {
            ...
            var effect = Stage.instance.SkillController.Get(pos, elementalType);
            effect.Play();
        ...
        public IEnumerator CoDoubleAttack(int[] damageMap, bool[] criticalMap)
        {
            ...
            var effect = Stage.instance.SkillController.Get<SkillDoubleVFX>(go, ElementalType.Fire, SkillCategory.DoubleAttack, SkillTargetType.Enemy);
            for (var i = 0; i < 2; i++)
            {
                var first = i == 0;
        ...
        public IEnumerator CoBuff(StatBuff buff)
        {
            ...
            var effect = Stage.instance.BuffController.Get<Player, BuffVFX>(_target, buff);
            effect.Play();
            yield return new WaitForSeconds(0.6f);
        }
        ...
        private IEnumerator CoAnimationBuffCast(StatBuff buff)
        {
            ...
            Animator.Cast();
            var pos = transform.position;
            var effect = Stage.instance.BuffController.Get(pos, buff);
            effect.Play();
            yield return new WaitForSeconds(0.6f);
        }
        ...
        public IEnumerator CoFinisher(int[] damageMap, bool[] criticalMap)
        {
            ...
            position.z = 0f;
            var effect = Stage.instance.objectPool.Get<FenrirSkillVFX>(position);
            effect.Stop();
