...
//                Debug.LogWarning($"{Animator.Target.name}'s {nameof(ProcessHeal)} called: {CurrentHP}({Model.Stats.CurrentHP}) / {HP}({Model.Stats.LevelStats.HP}+{Model.Stats.BuffStats.HP})");
        ...
        private void ProcessBuff(CharacterBase target, Model.BattleStatus.Skill.SkillInfo info)
        {
            if (target && !info.Target!.IsDead)
            {
                ...
                var force = new Vector3(-0.1f, 0.5f);
                var buff = info.Buff;
                var effect = Stage.instance.BuffController.Get<CharacterBase, BuffVFX>(target, buff);
            ...
            IReadOnlyList<Model.BattleStatus.Skill.SkillInfo> infos)
        {
            ...
            {
                var effect = Stage.instance.SkillController.GetBlowCasting(
                    pos,
                    info.SkillCategory,
                    info.ElementalType);
                effect.Play();
        ...
        protected virtual IEnumerator CoAnimationCast(Model.BattleStatus.Skill.SkillInfo info)
        {
            ...
            Animator.Cast();
            var pos = transform.position;
            var effect = Stage.instance.SkillController.Get(pos, info.ElementalType);
            effect.Play();
        ...
        private IEnumerator CoAnimationBuffCast(Model.BattleStatus.Skill.SkillInfo info)
        {
            ...
            Animator.Cast();
            var pos = transform.position;
            var effect = Stage.instance.BuffController.Get(pos, info.Buff);
            effect.Play();
#if TEST_LOG
            ...
            IReadOnlyList<Model.BattleStatus.Skill.SkillInfo> skillInfos)
        {
            ...
            for (var i = 0; i < skillInfosCount; i++)
            {
                ...
                if (info.ElementalType != ElementalType.Normal)
                {
                    var effect = Stage.instance.SkillController.Get<SkillBlowVFX>(target, info);
                    if (effect is null)
                        continue;
                    effect.Play();
                }
            ...
            IReadOnlyList<Model.BattleStatus.Skill.SkillInfo> skillInfos)
        {
            ...
            for (var i = 0; i < skillInfosCount; i++)
            {
                ...
                var effectObj = Stage.instance.objectPool.Get($"TwinAttack_0{i + 1}", false, effectPos) ??
            ...
            IReadOnlyList<Model.BattleStatus.Skill.SkillInfo> skillInfos)
        {
            ...
            for (var i = 0; i < skillInfosCount; i++)
            {
                ...
                var first = skillInfosFirst == info;
                var effect = Stage.instance.SkillController.Get<SkillDoubleVFX>(target, info);
                if (effect is null)
                    continue;
            ...
            IReadOnlyList<Model.BattleStatus.Skill.SkillInfo> skillInfos)
        {
            ...
            var effectTarget = Stage.instance.GetCharacter(skillInfosFirst.Target);
            if (effectTarget is null)
                yield break;

            var effect =
                Stage.instance.SkillController.Get<SkillAreaVFX>(effectTarget,
                    skillInfosFirst);
            if (effect is null)
                yield break;
        ...
        private IEnumerator CoExecuteAction()
        {
            if (action is null)
            {
                ...
                foreach (var info in action.skillInfos)
                {
                    ...
                    if (target.IsDead)
                    {
                        var character = Stage.instance.GetCharacter(target);
                        if (character)
                        {
