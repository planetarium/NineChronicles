...
namespace Nekoyume.BlockChain
{
    ...
    public class RPCAgent : MonoBehaviour, IAgent, IActionEvaluationHubReceiver
    {
        ...
        private void Awake()
        {
            OnDisconnected
                .ObserveOnMainThread()
                .Subscribe(_ => Analyzer.Instance.Trace("Unity/RPC Disconnected", new Dictionary<string, string>()
                {
            ...
            OnRetryStarted
                .ObserveOnMainThread()
                .Subscribe(_ => Analyzer.Instance.Trace("Unity/RPC Retry Connect Started", new Dictionary<string, string>()
                {
                    ["AvatarAddress"] = States.Instance.CurrentAvatarState.address.ToString(),
                    ["AgentAddress"] = States.Instance.AgentState.address.ToString(),
                }))
            ...
            OnRetryEnded
                .ObserveOnMainThread()
                .Subscribe(_ => Analyzer.Instance.Trace("Unity/RPC Retry Connect Ended", new Dictionary<string, string>()
                {
                    ["AvatarAddress"] = States.Instance.CurrentAvatarState.address.ToString(),
                    ["AgentAddress"] = States.Instance.AgentState.address.ToString(),
                }))
            ...
            OnPreloadStarted
                .ObserveOnMainThread()
                .Subscribe(_ => Analyzer.Instance.Trace("Unity/RPC Preload Started", new Dictionary<string, string>()
                    {
                        ["AvatarAddress"] = States.Instance.CurrentAvatarState.address.ToString(),
                    }))
                .AddTo(_disposables);
            OnPreloadEnded
                .ObserveOnMainThread()
                .Subscribe(_ => Analyzer.Instance.Trace("Unity/RPC Preload Ended", new Dictionary<string, string>()
                {
                    ["AvatarAddress"] = States.Instance.CurrentAvatarState.address.ToString(),
                    ["AgentAddress"] = States.Instance.AgentState.address.ToString(),
                }))
