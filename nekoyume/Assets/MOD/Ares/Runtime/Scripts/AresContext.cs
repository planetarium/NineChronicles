...
namespace NineChronicles.MOD.Ares
{
    public class AresContext
    {
        ...
        #region UI
        public VisualElement CurrentUI { get; set; }
        public int ArenaScoreBoardPage { get; set; }
        public readonly Dictionary<Address, float> WinRates = new();
        #endregion UI
        ...
        public async UniTask<float> GetWinRateAsync(Address enemyAvatarAddress)
        {
            if (WinRates.TryGetValue(enemyAvatarAddress, out var winRate))
            {
                return winRate;
            }
            ...
            var enemyRuneStates = await Agent.GetRuneStatesAsync(
                enemyAvatarAddress,
                runeSlotInfos: null);
            winRate = ArenaCalculator.ExecuteArena(
                sheets: Sheets,
                randomSeed: null,
                ...
                playCount: 100);
            WinRates[enemyAvatarAddress] = winRate;
            return winRate;
        }
    }
}
